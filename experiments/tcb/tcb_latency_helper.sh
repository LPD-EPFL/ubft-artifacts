...
for msg_sz in ${MSG_SZ[@]}; do
    ...
    ssh -o LogLevel=QUIET -t $(machine2ssh machine1) "$ROOT_DIR/ubft_experiment/$(machine2dir machine1)/deployment/invoker.sh binaries/tail-cb-ping ubft-tcb-test -l 1 $FASTPATH_ON -s ${msg_sz}"
    ssh -o LogLevel=QUIET -t $(machine2ssh machine2) "$ROOT_DIR/ubft_experiment/$(machine2dir machine2)/deployment/invoker.sh binaries/tail-cb-ping ubft-tcb-test -l 2 $FASTPATH_ON -s ${msg_sz}"
    ssh -o LogLevel=QUIET -t $(machine2ssh machine3) "$ROOT_DIR/ubft_experiment/$(machine2dir machine3)/deployment/invoker.sh binaries/tail-cb-ping ubft-tcb-test -l 3 $FASTPATH_ON -s ${msg_sz}"

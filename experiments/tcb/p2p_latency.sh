...
for msg_sz in ${MSG_SZ[@]}; do
    ...
    ssh -o LogLevel=QUIET -t $(machine2ssh machine1) "$ROOT_DIR/ubft_experiment/$(machine2dir machine1)/deployment/invoker.sh binaries/tail-p2p-ping tail-p2p-ping -l 1 -s $msg_sz"
    ssh -o LogLevel=QUIET -t $(machine2ssh machine2) "$ROOT_DIR/ubft_experiment/$(machine2dir machine2)/deployment/invoker.sh binaries/tail-p2p-ping tail-p2p-ping -l 2 -s $msg_sz"

...
sgx_status_t ecall_usig_create_ui_shared_key(uint8_t *input, size_t cnt,
                                  uint64_t *counter,
                                  hmac_t hmac)
{
        static uint64_t usig_counter = 1;
        ...
        blake3_hasher_init_keyed(&hasher, usig_shared_key);

        blake3_hasher_update(&hasher, &usig_counter, sizeof(uint64_t));
        blake3_hasher_update(&hasher, input, cnt);
        *counter = usig_counter;

        // Increment the internal counter just before going to expose
        // a valid signature to the untrusted world. That makes sure
        // the counter value cannot be reused to sign another message.
        usig_counter++;
        blake3_hasher_finalize(&hasher, hmac, BLAKE3_OUT_LEN);
        ret = SGX_SUCCESS;
out:
        return ret;
}

sgx_status_t ecall_usig_verify_ui_shared_key(uint64_t *counter, hmac_t hmac, uint8_t *input, size_t cnt,
                                  bool *valid)
{
        sgx_status_t ret;
        blake3_hasher hasher;
        hmac_t expected_hmac;
        ...
        blake3_hasher_update(&hasher, counter, sizeof(uint64_t));
        blake3_hasher_update(&hasher, input, cnt);
        blake3_hasher_finalize(&hasher, expected_hmac, BLAKE3_OUT_LEN);
        
        *valid = memcmp(hmac, expected_hmac, BLAKE3_OUT_LEN) == 0;
...
sgx_status_t ecall_usig_init(void *sealed_data, uint32_t sealed_data_size, size_t given_cpu_features)
{
        sgx_status_t ret;
        ...
        // ret = sgx_read_rand((void *)usig_shared_key, sizeof(usig_shared_key));
        // if (ret != SGX_SUCCESS) {
        //         goto out;
        // }
        memset(usig_shared_key, 96, BLAKE3_KEY_LEN);
        ...
        cpu_features = given_cpu_features;
        initialized = true;
